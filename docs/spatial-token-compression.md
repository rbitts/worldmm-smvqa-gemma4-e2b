...
> Legacy migration source. The canonical living architecture is
> [spatial-memory/architecture.md](spatial-memory/architecture.md). Do not add new
> results or design decisions to this page.
...
subject to
    ...
    records(M_static_window) <= token_budget
    serialized_bytes(M_static_window) <= byte_budget
...
- QA utility, geometry fidelity, storage, write frequency 사이 Pareto frontier를
  측정해야 한다.
- 현재 hard budget의 대상은 static `SpatialTokenRecord`다. Trajectory summary를
  포함한 전체 artifact byte 제한은 아직 구현되지 않았다.
...
`min_keep_score=0.5`를 통과한 후보를 `end_time` 순서로 admission한다. 같은
관측시각 후보만 selector score를 실제 직렬화 byte로 나눈 값이 큰 순서로
greedy admission한다. Byte 비용은 importance를 반영한 전체
`SpatialTokenRecord.model_dump_json()`의 UTF-8 길이와 JSONL newline 1 byte다.

한 window에서 `token_budget`과 `byte_budget`을 동시에 만족해야 한다. 기본값은
30초당 static spatial token 최대 16개, 4096 byte다. Variable-length label,
frame reference, provenance 같은 record metadata도 byte 비용에 포함된다.
...
주의: 두 budget 모두 `SpatialTokenRecord`에만 적용된다. Trajectory summary는
별도로 추가되므로 전체 spatial artifact record 수나 byte에 대한 strict upper
bound는 아니다.
...
Static token admission은 30초 window마다 record 수와 실제 serialized JSONL
byte를 모두 제한한다. 그러나 trajectory summary는 admission 뒤 추가되므로
전체 spatial artifact byte budget은 보장하지 않는다. Full-artifact cap에는
trajectory 비용 포함 또는 별도 trajectory budget이 필요하다.
...
현재 cap은 window별 static-token write 수와 byte만 제한하며 전체 lifetime
storage bound는 아니다.
...
ratio, spatial token count를 QA metric과 함께 기록한다. 여기서
`compressed_bytes`는 trajectory까지 포함한 전체 output 측정값이며, admission의
4096-byte cap은 static `SpatialTokenRecord` 부분에만 적용된다.
