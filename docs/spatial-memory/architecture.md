...
| Geometry teacher | G-CUT3R-compatible model은 company-side offline oracle/target 생성에만 사용 |
| Device compiler | Small RGB semantics + native causal IMU/VIO/depth + deterministic geometry |
| Storage control | Canonical serialized byte에 hard limit 적용 |
| Spatial answer | Deterministic executor가 proof 또는 abstention 생성 |
| 현재 경계 | Object/place vertical slice local contract만 구현; raw student 없음 |
...
```text
offline training/evaluation lane
    1 Hz RGB + calibration + optional pose/depth
    -> G-CUT3R-compatible teacher
    -> transient point/depth/confidence
    -> selected evidence-bound object/place targets

on-device lane (oracle Go 이후)
    1 Hz RGB -> small detector/semantic encoder
    native-rate IMU/VIO + optional calibrated depth -> deterministic projection
    -> causal object association
    -> object / place-label / event / no-write candidates
...
heuristic으로 대체한다. Teacher target compiler는 external semantic point selection을
입력받으며 raw image segmentation을 구현하지 않는다.
...
|---|---|---|
| Place/submap (future) | Local SE(3) anchor, covariance, topology | Coordinate ownership, loop correction |
...
| Object | Entity/instance ID, centroid, extent, orientation, optional semantic place | Metric·object/location QA |
...
현재 schema에는 별도 Place record가 없다. Object의 optional `place_label`과
`local_frame_id`가 EXP-0005 최소 vertical slice이며 topology 효용이 확인될 때만
Place/submap type을 추가한다.

## Legacy production bridge 구조
...
이 bridge는 lineage/validation scaffold다. `spatial_train.py`의 supplied-vector
candidate head와 함께 raw-sensor student 또는 device architecture를 구성하지 않는다.
...
| Evidence grounding | Observed/fused/model-inferred/human record는 validity 내 selected same-video frame을 인용하고 min/max evidence time과 first/last seen, unique count와 `observation_count`가 일치 |
...
| Geometry proof | Entity role, operation, value, frame, unit, uncertainty, provenance, evidence, behavior-affecting option을 stable hash에 포함; model inference는 confidence gate 추가 |
...
`last_seen`, `last_location`, `count`다. Missing frame, ambiguous entity,
missing/low-confidence inferred place, incompatible frame,
...
Repository는 causal sensor schema, teacher/cache contract, selected-point object
target compiler, typed schema, hard byte validation, retrieval, proof, QA check,
lineage를 소유한다. Production G-CUT3R extractor, semantic mask/place provider, raw
sensor encoder, learned open-world association, mobile executable은 소유하지 않는다.

다음 실행은 [EXP-0005](experiments/exp-0005-teacher-oracle-ceiling.md)다. Oracle이
...
- [ADR-0005: Hybrid on-device compiler](decisions/adr-0005-hybrid-on-device-compiler.md)
- [ADR-0006: Evidence-bound inferred geometry](decisions/adr-0006-evidence-bound-inferred-geometry.md)
