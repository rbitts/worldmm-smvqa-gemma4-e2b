...
class SpatialUncertainty(FrozenModel):
    ...
    def _require_positive_semidefinite_covariance(self) -> Self:
        ...
        if any(
            covariance[index][index] < -(_COVARIANCE_TOLERANCE * scale)
            for index in range(3)
        ):
            msg = "covariance must be positive semidefinite"
            raise ValueError(msg)
        second_order_tolerance = _COVARIANCE_TOLERANCE * (scale**2)
        if any(
            (
        ...
        if determinant < -(_COVARIANCE_TOLERANCE * (scale**3)):
            msg = "covariance must be positive semidefinite"
            raise ValueError(msg)
        return self
